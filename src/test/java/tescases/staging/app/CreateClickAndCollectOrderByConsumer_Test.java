...
public class CreateClickAndCollectOrderByConsumer_Test extends BaseTest {
...
//        open("https://maltipoo-dk.staging-bemakers.shop/dk");
        open("https://maltipoo-dk.staging-bemakers.shop");
...
//        $("#confirm-age").hover().click();
        ...
        $("a[href='/dk/products/wine").hover().click();
        Product_Page product_page = new Product_Page();
        $("h1.h1").shouldHave(text("Wine"));
        $("h1+p").shouldHave(text("12,50"));
...
//        $("button[type='submit']").hover().click();
        ...
        if(true)//if physical product true
        {
        ...
        //shipping PostNord - collect
        if(true)//clickAndCollectOrder
            {
                waitForPageIsLoaded();
                checkout_page.postnordCollect.hover().click();
                waitForPageIsLoaded();
                $(By.xpath("//span[contains(text(),'Nærboks Amager Strandvej 60 - Kræver Postnord App')]")).hover().click(); // value MTpTZXJ2aWNlUG9pbnQ6MjA2
             }
        waitForPageIsLoaded();
        checkout_page.continue_Btn.hover().click();
        //payment Credit Card
        waitForPageIsLoaded();
        if(false){// billing address is required
...
//        login_page.email_Input.setValue("alex+andersenwinery@bemakers.com");
        ...
        open(ReceiveMailApi.getConfirmOrderLink(orderNumber));
